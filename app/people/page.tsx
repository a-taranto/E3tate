...
export default function BeneficiariesPage() {
  ...
  const openAdd = () => {
    setEditingId(null);
    setForm({ name: "", email: "", role: "beneficiary", relationship: "", residentialAddress: "" });
    setShowForm(true);
  };
  ...
  const submitForm = () => {
    ...
    logActivity(
      editingId ? "Person Updated" : "Person Added",
      "beneficiary",
      `${form.name} (${form.role})`
    );
  ...
  const removeBeneficiary = (person: Beneficiary) => {
    if (!confirm(`Remove ${person.name} from your people?`)) return;
    const updated = beneficiaries.filter((b) => b.id !== person.id);
    setBeneficiaries(updated);
    saveBeneficiaries(updated);
    logActivity("Person Removed", "beneficiary", person.name);
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="md:col-span-2">
              <Input
                ...
                placeholder="42 Park Ave, Sydney NSW 2000"
      ...
      {/* Executor-required guidance */}
      {loaded && executorCount === 0 && (
        <Card className="mb-6 border-l-4" style={{ borderLeftColor: "var(--warning)" }}>
          <div className="flex items-start gap-3">
            <Shield className="h-5 w-5 mt-0.5 flex-shrink-0" style={{ color: "var(--warning)" }} />
            <div className="flex-1">
              <p className="font-medium" style={{ color: "var(--text-primary)" }}>
                You need at least one executor
              </p>
              <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
                An executor manages your estate and is appointed as Executor &amp; Trustee in your will.
              </p>
            </div>
        ...
        {!loaded ? (
          ...
          <Card className="text-center py-12">
            ...
            <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>
              Add the executors, beneficiaries, guardians, and trusted contacts involved in your estate.
            </p>
            <Button variant="primary" onClick={openAdd}>
              <UserPlus className="h-4 w-4" />
